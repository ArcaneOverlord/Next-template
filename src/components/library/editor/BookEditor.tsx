...
import { 
  ...
  RefreshCcw, Download, FileUp, Heading1 
} from 'lucide-react';
import CanvasBlockComponent from './CanvasBlock';

// ------------------------------------------------------------------
// 1. MODALS (Table & Heading)
// ------------------------------------------------------------------
...
function HeadingPromptModal({ onGenerate, onCancel }: { onGenerate: (power: number) => void; onCancel: () => void }) {
  const [power, setPower] = useState(1);
  const powers = [
    { level: 1, label: 'Title / H1' },
    { level: 2, label: 'Section / H2' },
    { level: 3, label: 'Subsection / H3' },
    { level: 4, label: 'Segment / H4' },
    { level: 5, label: 'Clause / H5' },
    { level: 6, label: 'Sub-clause / H6' },
    { level: 7, label: 'Point / H7' },
    { level: 8, label: 'Detail / H8' },
  ];
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-[100] flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-sm p-6 shadow-2xl">
        <div className="flex justify-between items-center mb-4">
          <h3 className="font-bold text-lg text-gray-800">Add Heading</h3>
          <button onClick={onCancel} className="text-gray-400 hover:text-gray-600"><X size={20} /></button>
        </div>
        <div className="space-y-4 mb-6">
          <label className="block text-sm font-semibold mb-1 text-gray-700">Select Power Level</label>
          <select 
            value={power} 
          ...
          >
            {powers.map(p => (
              <option key={p.level} value={p.level}>Level {p.level}: {p.label}</option>
            ))}
          </select>
        </div>
        <button 
          onClick={() => onGenerate(power)} 
          className="w-full bg-blue-600 text-white font-bold py-2 rounded hover:bg-blue-700 transition"
        >
          Create Heading
        </button>
...
// 2. MAIN EDITOR
// ------------------------------------------------------------------
export type BlockType = 'text' | 'media' | 'table' | 'heading';
...
export default function BookEditor({ bookId, bookName }: BookEditorProps) {
  ...
  const [showHeadingPrompt, setShowHeadingPrompt] = useState(false);
  ...
  // --- ACTIONS ---
  const addBlock = (type: BlockType, tableRows = 2, tableCols = 2, prefillContent = '', tableName = '', powerLevel = 8) => {
    ...
    const newBlock: CanvasBlock = {
      ...
      scrollMode: 'grow',
      title: tableName,
      powerLevel: type === 'heading' ? powerLevel : (type === 'text' ? 8 : undefined)
    };
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 flex flex-col h-[calc(100vh-8rem)] relative overflow-hidden">
            ...
            <>
              ...
              <div className="relative">
                ...
                {showAddMenu && (
                  <div className="absolute right-0 mt-2 w-48 bg-white border border-gray-200 shadow-xl rounded-xl py-2 z-50 origin-top-right">
                    <button 
                      onClick={() => { setShowHeadingPrompt(true); setShowAddMenu(false); }} 
                      className="w-full flex items-center px-4 py-3 text-sm text-gray-700 hover:bg-gray-50"
                    >
                      <Heading1 size={16} className="mr-3 text-indigo-600" /> Heading Box
                    </button>
                    <button 
              ...
              <div className="relative">
                ...
                {isMenuOpen && (
                  <div className="absolute right-0 mt-2 w-48 bg-white border border-gray-200 shadow-xl rounded-xl py-2 z-50 origin-top-right">
                    ...
                    >
                      <Move size={16} className="mr-3" /> Transform Menu
                    </button>
                    <button 
      ...
      {showHeadingPrompt && (
        <HeadingPromptModal 
          onGenerate={(power) => { addBlock('heading', 0, 0, '', '', power); setShowHeadingPrompt(false); }} 
          onCancel={() => setShowHeadingPrompt(false)} 
        />
      )}
