      ...
      <div className="bg-white rounded-2xl w-full max-w-sm p-6 shadow-2xl">
        ...
        <div className="space-y-4 mb-6">
          <div>
            ...
            <input type="text" value={tableName} onChange={(e) => setTableName(e.target.value)} className="w-full p-2 border border-gray-300 rounded text-gray-900 outline-none focus:border-blue-500" placeholder="e.g., User Data" />
          </div>
          <div className="flex space-x-4">
            <div className="flex-1"><label className="block text-sm font-semibold mb-1 text-gray-700">Rows</label><input type="number" min="1" max="20" value={rows} onChange={(e) => setRows(Number(e.target.value))} className="w-full p-2 border border-gray-300 rounded text-center text-gray-900 outline-none" /></div>
            <div className="flex-1"><label className="block text-sm font-semibold mb-1 text-gray-700">Cols</label><input type="number" min="1" max="10" value={cols} onChange={(e) => setCols(Number(e.target.value))} className="w-full p-2 border border-gray-300 rounded text-center text-gray-900 outline-none" /></div>
          </div>
        </div>
...
export interface CanvasBlock { 
  ...
  scrollMode: ScrollMode; title?: string;
}

export default function BookEditor({ bookId, bookName }: { bookId: string; bookName: string; }) {
  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      if (menuRef.current && !menuRef.current.contains(event.target as Node)) { setIsMenuOpen(false); setShowAddMenu(false); }
  ...
  const addBlock = (type: BlockType, tableRows = 2, tableCols = 2, prefillContent = '', tableName = '') => {
    ...
    if (type === 'table' && !prefillContent) content = JSON.stringify(Array(tableRows).fill(Array(tableCols).fill('')));
    
    const marginX = 16;
    const canvasW = canvasRef.current ? canvasRef.current.clientWidth : window.innerWidth;
    
    // Auto Snug Fit
    let blockWidth = Math.min(canvasW - (marginX * 2), type === 'table' ? 600 : 450);
    let blockHeight: number | string = 'auto';
    if (type === 'media') blockHeight = 250;
    ...
    const newBlock: CanvasBlock = {
      ...
      type, content, x: marginX, y: startY,
      w: blockWidth, h: blockHeight, scrollMode: 'grow', title: tableName
    };
  ...
  const handleImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.name.endsWith('.xlsx') || file.name.endsWith('.csv')) {
      ...
      addBlock('media', 0, 0, URL.createObjectURL(file)); 
    } else {
  ...
  const handleResizeDown = (e: React.PointerEvent, id: string, handle: string) => {
    ...
    if (block) {
      ...
      setResizing({ id, handle, initialRect: { x: block.x, y: block.y, w: block.w, h: block.h === 'auto' ? elHeight : block.h } });
  ...
  const handlePointerUp = () => {
    ...
    // Mathematical Anti-Overlap Engine (No DOM loops = No freezing)
    ...
    if (activeBlock) {
      ...
      const dH = activeBlock.h === 'auto' ? document.getElementById(`block-${activeBlock.id}`)?.offsetHeight || 100 : activeBlock.h as number;

      while (hasOverlap && iterations < 15) {
        hasOverlap = false;
        for (const b of blocks) {
          if (b.id === draggingBlock) continue;
          const bH = b.h === 'auto' ? document.getElementById(`block-${b.id}`)?.offsetHeight || 100 : b.h as number;
          
          // Pure Math Intersection
          if (!(activeBlock.x + dW < b.x || activeBlock.x > b.x + b.w || finalY + dH < b.y || finalY > b.y + bH)) {
            hasOverlap = true;
            finalY = b.y + bH + 16; // Snap below it
            break;
          }
        }
        iterations++;
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 flex flex-col h-[calc(100vh-8rem)] relative overflow-hidden">
          ...
          {appMode === 'edit' && (
            <>
              ...
              <div className="relative">
                ...
                {showAddMenu && (
                  <div className="absolute right-0 mt-2 w-48 bg-white border border-gray-200 shadow-xl rounded-xl py-2 z-50 origin-top-right">
                    ...
                    <button onClick={() => fileInputRef.current?.click()} className="w-full flex items-center px-4 py-3 text-sm text-blue-600 hover:bg-blue-50 font-medium"><FileUp size={16} className="mr-3" /> Import File...</button>
              ...
              <div className="relative">
                ...
                {isMenuOpen && (
                  <div className="absolute right-0 mt-2 w-48 bg-white border border-gray-200 shadow-xl rounded-xl py-2 z-50 origin-top-right">
                    <button onClick={() => { setTransformBackup(JSON.parse(JSON.stringify(blocks))); setAppMode('transform'); setIsMenuOpen(false); }} className="w-full flex items-center px-4 py-3 text-sm font-bold text-indigo-700 hover:bg-indigo-50"><Move size={16} className="mr-3" /> Transform Menu</button>
      ...
      <div 
        ref={canvasRef}
        className={`flex-1 relative overflow-auto p-4 ${appMode === 'read' ? 'bg-white' : appMode === 'transform' ? 'bg-gray-100' : 'bg-gray-50'}`}
      ...
      >
        {blocks.filter(b => b.content.toLowerCase().includes(searchQuery.toLowerCase())).map((block) => (
          <CanvasBlockComponent 
            ...
            onMenuClick={(x, y, id) => setContextMenu({ visible: true, x, y, blockId: id })}
          />
        ))}
      </div>
